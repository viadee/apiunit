 ...
 *
 *    Copyright 2023 viadee IT-Unternehmensberatung AG
 *
...
public class HttpDeleteMethodsMustNotHaveRequestBody implements ApiRule {
    ...
    public ApiUnitTestResultObject check(JavaClasses importedClasses) {
        for (JavaClass importedClass : importedClasses) {
            checkMethodsForDeleteMethodsForRequestBody(importedClass);
        }
    ...
    private void checkMethodsForDeleteMethodsForRequestBody(JavaClass javaClass) {
        javaClass.getAllMethods().forEach(javaMethod -> {
            if (Utils.isDeleteMethod(javaMethod)) {
